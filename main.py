...
def escreve_df(lista_candidato_completo: list[CandidatoCompleto], ano_eleicao: int, lista_candidato_erro: list[CandidatoDetail]):
    ...
    df = pd.DataFrame(
        data=[(item.nome_completo, item.cargo.nome, item.local_candidatura, item.uf_candidatura, item.cnpjcampanha, item.descricao_sexo, item.descricao_cor_raca, item.nacionalidade, item.grau_instrucao, item.gasto_campanha1_t, item.gasto_campanha2_t,
               item.numero_processo_drap, item.numero_processo,  item.numero_processo_prest_contas, item.id, item.numero, item.ocupacao, ano_eleicao, 1 if item.bens else 0)
        ...
        columns=["nome_completo", "cargo", "localCandidatura", "ufCandidatura", "cnpjcampanha", "descricaoSexo", "descricaoCorRaca", "nacionalidade", "grauInstrucao", "gastoCampanha1T", "gastoCampanha2T",
                 "numeroProcessoDrap", "numeroProcesso", "numeroProcessoPrestContas", "id", "numero", "ocupacao", "anoEleicao", "fl_bens"])
    ...
    for item in lista_candidato_completo:
        ...
        for bem in item.bens:
            bens.append([item.id, bem.ordem, bem.descricao, bem.descricao_de_tipo_de_bem, bem.valor, bem.data_ultima_atualizacao,ano_eleicao])
    ...
    df_bens = pd.DataFrame(
        data=bens,
        columns=["idUsuario", "ordem", "descricao", "descricao_de_tipo_de_bem", "valor", "data_ultima_atualizacao", "anoEleicao"])
    ...
    df_bens_excel = pd.read_excel('dados_combinados.xlsx', sheet_name='Bens')
    ...
    df_bens = pd.concat(
        [df_bens if not df_bens.empty else None, df_bens_excel])
    # df_erro = pd.concat(
    ...
    with pd.ExcelWriter('dados_combinados.xlsx') as writer:
        ...
        df_bens.to_excel(writer, index=False, sheet_name='Bens')
