...
from models.candidato_model import Candidato
...
from utils.constants import SCHEMA, EXTRAIR_NOVAMENTE
...
def escreve_df(lista_candidato_completo: list[CandidatoCompleto], ano_eleicao: int):
    ...
    df = pd.DataFrame(
        data=[(item.nome_completo, item.cargo.nome, item.local_candidatura, item.uf_candidatura, item.cnpjcampanha, item.descricao_sexo, item.descricao_cor_raca, item.nacionalidade, item.grau_instrucao, item.gasto_campanha1_t, item.gasto_campanha2_t,
               item.numero_processo_drap, item.numero_processo,  item.numero_processo_prest_contas, item.id)
        ...
        columns=["nome_completo", "cargo", "localCandidatura", "ufCandidatura", "cnpjcampanha", "descricaoSexo", "descricaoCorRaca", "nacionalidade", "grauInstrucao", "gastoCampanha1T", "gastoCampanha2T",
                 "numeroProcessoDrap", "numeroProcesso", "numeroProcessoPrestContas", "id"])
...
extrair_novamente = EXTRAIR_NOVAMENTE
...
if extrair_novamente == 'S':
    ...
    # print(eleicoes[idx_selecionado].ano)
    # print(eleicoes[idx_selecionado].id)
    ...
    estados_municipios = EstadoMunicipio.from_dict(json.loads(res_municipios.text))
    ...
    for index, municipio in enumerate(estados_municipios.municipios):
        ...
        for index, cargo in enumerate(cargos.cargos):
            ...
            for index, candidato in enumerate(candidatos.candidatos):
                ...
                res_candidatos_completo = requests.get(url_candidatos_completo)
                ...
                #     print(e)
                #     lista_erro_candidato_completo.append(candidatos)
